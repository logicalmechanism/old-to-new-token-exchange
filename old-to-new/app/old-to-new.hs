...
import           OldToNew (oldToNewScript, oldToNewScriptShortBs)
...
main = do
  ...
  let scriptname = if nargs > 1 then args!!1 else  "old_to_new.plutus"
  putStrLn $ "OUTPUT: " ++ scriptname
  writePlutusScript scriptnum scriptname oldToNewScript oldToNewScriptShortBs
